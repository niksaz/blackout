...
public class ParticleSpecialEffect implements SpecialEffect {
    ...
    protected ParticleSpecialEffect(ParticleEffect effect, float x, float y) {
        this.effect = effect;
     ...
     */
    public static void createWithCopy(ParticleEffect effect, float x, float y) {
        create(effect.copy(), x, y);
    }

    /**
     * This constructor will NOT copy the <code>ParticleEffect</code> but it will <code>dispose</code> it after usage.
     */
    public static void create(ParticleEffect effect, float x, float y) {
        BlackoutGame.get().specialEffects().add(new ParticleSpecialEffect(effect, x, y));
    }

    /**
     * Just calls <code>ParticleSpecialEffect.create(effect, position.x, position.y);</code>
     */
    public static void create(ParticleEffect effect, Vector2 position) {
        create(effect, position.x, position.y);
    }
